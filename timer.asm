...
InitTimers:
	...
	LDI		XL, LOW(delay_timer)	;point to the timer data
	LDI		XH, HIGH(delay_timer)
...
SetupDelayTimer:			;setup the delay timer
	...
	LDI		YL, LOW(delay_timer)	;get the address of the timer
	LDI		YH, HIGH(delay_timer)
	ADD		YL, R16
...
StartTmrHandler:			;update the delay timers if non-zero
	LDI		XL, LOW(delay_timer)	;get the timer address
	LDI		XH, HIGH(delay_timer)
...
delay_timer:    .byte	2 * TIMER_CNT	; array of delay timers (2 bytes/timer)
